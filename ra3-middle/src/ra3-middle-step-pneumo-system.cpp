...
void RA3Middle::stepPneumoSystem(double t, double dt)
{
    ...
    double Q_pm_fwd = K_pm_fwd * pf(forward_inputs[SME_FWD_PM_PRESSURE] - main_res->getPressure());

    // Расчитываем переток воздуха из ПМ заднего вагона
    double K_pm_bwd = 0.05;
    double Q_pm_bwd = K_pm_bwd * pf(backward_inputs[SME_BWD_PM_PRESSURE] - main_res->getPressure());
