...
void RA3HeadMotor::stepDisel(double t, double dt)
{
    if (is_active)
        ...
        disel->setRefFreq(max(backward_inputs[SME_DISEL_FREQ], forward_inputs[SME_DISEL_FREQ]));

    disel->setMV6state(mpsu->getOutputData().is_fuel_valve1_open ||
                       static_cast<bool>(backward_inputs[SME_BWD_FUEL_VALVE_OPEN]) ||
    ...
    bool is_starter_relay_ON = mpsu->getOutputData().is_starter1_ON ||
            static_cast<bool>(backward_inputs[SME_BWD_STARTER_ON]) ||
            static_cast<bool>(forward_inputs[SME_BWD_STARTER_ON]);
    starter_relay->setVoltage(Ucc_24 * static_cast<double>(is_starter_relay_ON));
